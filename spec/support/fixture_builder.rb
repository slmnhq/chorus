...
FixtureBuilder.configure do |fbuilder|
  ...
  fbuilder.factory do
    ...
    fbuilder.name :bob_user_added_event, Events::UserAdded.by(admin).add(:new_user => owner)
    ...
    bobs_instance = GpdbInstance.create!({ :name => "bobs_instance", :description => "Bob-like", :host => "non.legit.example.com", :port => "5432", :maintenance_db => "postgres", :owner => owner, :shared => false}, :without_protection => true)
    fbuilder.name :bob_creates_greenplum_instance, Events::GreenplumInstanceCreated.by(owner).add(:greenplum_instance => bobs_instance)
    ...
    bob_bobs_instance_account = InstanceAccount.create!({:owner => owner, :gpdb_instance => bobs_instance, :db_username => 'bobo', :db_password => 'i <3 me'}, :without_protection => true)
    ...
    workspaces << bob_public_workspace = owner.owned_workspaces.create!({:name => "Bob Public", :summary => "searchquery", :sandbox => bob_schema}, :without_protection => true)
    workspaces << bob_private_workspace = owner.owned_workspaces.create!(:name => "Bob Private", :summary => "searchquery", :public => false)
    ...
    fbuilder.name :bob_creates_public_workspace, Events::PublicWorkspaceCreated.by(owner).add(:workspace => bob_public_workspace, :actor => owner)
    fbuilder.name :bob_creates_private_workspace, Events::PrivateWorkspaceCreated.by(owner).add(:workspace => bob_private_workspace, :actor => owner)

    fbuilder.name :bob_makes_workspace_public, Events::WorkspaceMakePublic.by(owner).add(:workspace => bob_public_workspace, :actor => owner)
    fbuilder.name :bob_makes_workspace_private, Events::WorkspaceMakePrivate.by(owner).add(:workspace => bob_private_workspace, :actor => owner)
    ...
    File.open(Rails.root.join('spec', 'fixtures', 'workfile.sql')) do |file|
      ...
      bob_private = Workfile.create!({:file_name => "Bob Private", :description => "searchquery", :owner => owner, :workspace => bob_private_workspace, :execution_schema => bob_schema}, :without_protection => true)
      bob_public = Workfile.create!({:file_name => "Bob Public", :description => "searchquery", :owner => owner, :workspace => bob_public_workspace}, :without_protection => true)
      ...
      sql_workfile = Workfile.create!({:file_name => "sql.sql", :owner => owner, :workspace => bob_public_workspace}, :without_protection => true)
      ...
      WorkfileVersion.create!({:workfile => bob_private, :version_num => "1", :owner => owner, :modifier => owner, :contents => file}, :without_protection => true)
      WorkfileVersion.create!({:workfile => bob_public, :version_num => "1", :owner => owner, :modifier => owner, :contents => file}, :without_protection => true)
      ...
      fbuilder.name :bob_creates_public_workfile, Events::WorkfileCreated.by(owner).add(:workfile => bob_public, :workspace => bob_public_workspace)
      fbuilder.name :bob_creates_private_workfile, Events::WorkfileCreated.by(owner).add(:workfile => bob_private, :workspace => bob_private_workspace)
      ...
      fbuilder.name :note_on_bob_public_workfile, Events::NoteOnWorkfile.by(owner).add(:workspace => bob_public_workspace, :workfile => bob_public, :body => 'notesearch forever')
      ...
      Events::ChorusViewCreated.by(owner).add(:dataset => bob_chorus_view, :workspace => bob_public_workspace, :source_object => bob_public)
      Events::ChorusViewChanged.by(owner).add(:dataset => bob_chorus_view, :workspace => bob_public_workspace)
    end

    text_workfile = Workfile.create!({:file_name => "text.txt", :owner => owner, :workspace => bob_public_workspace}, :without_protection => true)
    image_workfile = Workfile.create!({:file_name => "image.png", :owner => owner, :workspace => bob_public_workspace}, :without_protection => true)
    binary_workfile = Workfile.create!({:file_name => "binary.tar.gz", :owner => owner, :workspace => bob_public_workspace}, :without_protection => true)
    ...
    import_schedule = ImportSchedule.create!({:start_datetime => '2012-09-04 23:00:00-07', :end_date => '2012-12-04', :frequency => 'weekly', :workspace_id => bob_public_workspace.id, :to_table => "new_table_for_import", :source_dataset_id => bobs_table.id, :truncate => 't', :new_table => 't', :user_id => owner.id} , :without_protection => true)
    ...
    Events::NoteOnGreenplumInstance.create!({:greenplum_instance => purplebanana_instance, :actor => owner, :body => 'is this a greenplumsearch instance?', :created_at => '2010-01-01 02:02'}, :without_protection => true)
    Events::NoteOnGreenplumInstance.create!({:greenplum_instance => purplebanana_instance, :actor => owner, :body => 'no, not greenplumsearch', :created_at => '2010-01-01 02:03'}, :without_protection => true)
    Events::NoteOnGreenplumInstance.create!({:greenplum_instance => purplebanana_instance, :actor => owner, :body => 'really really?', :created_at => '2010-01-01 02:04'}, :without_protection => true)
    ...
    Events::NoteOnWorkspace.by(owner).add(:workspace => bob_public_workspace, :body => 'Come see my awesome workspace!')
    Events::NoteOnDataset.by(owner).add(:dataset => bobs_table, :body => 'Note on dataset')
    Events::NoteOnWorkspaceDataset.by(owner).add(:dataset => bobs_table, :workspace => bob_public_workspace, :body => 'Note on workspace dataset')
    ...
    fbuilder.name :note_on_workspace_dataset, Events::NoteOnWorkspaceDataset.by(owner).add(:dataset => searchquery_table, :workspace => bob_public_workspace, :body => 'workspacedatasetnotesearch')
    fbuilder.name :note_on_bob_public, Events::NoteOnWorkspace.by(owner).add(:workspace => bob_public_workspace, :body => 'notesearch forever')
    ...
    Events::WorkspaceAddSandbox.by(owner).add(:sandbox_schema => bob_schema, :workspace => bob_public_workspace)
    ...
    Events::WorkspaceAddHdfsAsExtTable.by(owner).add(:workspace => bob_public_workspace, :dataset => bobs_table, :hdfs_file => hdfs_entry)
    Events::FileImportCreated.by(owner).add(:workspace => bob_public_workspace, :dataset => nil, :file_name => 'import.csv', :import_type => 'file', :destination_table => 'bobs_table')
    Events::FileImportSuccess.by(owner).add(:workspace => bob_public_workspace, :dataset => bobs_table, :file_name => 'import.csv', :import_type => 'file')
    Events::FileImportFailed.by(owner).add(:workspace => bob_public_workspace, :file_name => 'import.csv', :import_type => 'file', :destination_table => 'my_table', :error_message => "oh no's! everything is broken!")
    Events::MembersAdded.by(owner).add(:workspace => bob_public_workspace, :member => the_collaborator, :num_added => '5')
    Events::DatasetImportCreated.by(owner).add(:workspace => bob_public_workspace, :dataset => nil, :source_dataset => bobs_table, :destination_table => 'other_table')
    Events::DatasetImportSuccess.by(owner).add(:workspace => bob_public_workspace, :dataset => other_table, :source_dataset => bobs_table)
    Events::DatasetImportFailed.by(owner).add(:workspace => bob_public_workspace, :source_dataset => bobs_table, :destination_table => 'other_table', :error_message => "oh no's! everything is broken!")
    Events::ChorusViewCreated.by(owner).add(:dataset => bob_chorus_view, :workspace => bob_public_workspace, :source_object => bobs_table)
    Timecop.return
    ...
    #Notification
    bobs_notes = Events::NoteOnGreenplumInstance.by(owner)
    fbuilder.name(:bobs_notification1, Notification.create!({:recipient => owner, :notification_event => bobs_notes[0]}, :without_protection => true) )
    fbuilder.name(:bobs_notification2, Notification.create!({:recipient => owner, :notification_event => bobs_notes[1]}, :without_protection => true) )
    fbuilder.name(:bobs_notification3, Notification.create!({:recipient => owner, :notification_event => bobs_notes[2]}, :without_protection => true) )
    fbuilder.name(:bobs_notification4, Notification.create!({:recipient => owner, :notification_event => bobs_notes[3]}, :without_protection => true) )
  end
end
