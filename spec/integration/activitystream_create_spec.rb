...
describe " add an instance " do
  ...
  xit "creates an activity stream when a gpdb instance is created" do
  ...
  xit "creates an activity stream when a hadoop instance is created" do
    ...
    hadoop_instance_id = Instance.find_by_name("hadoop_instance").id
    go_to_home_page
    page.should have_content "EDC Admin added a new instance hadoop_instance"
    sleep(3)
    go_to_instance_page
    within(".instance_provider") do
      page.find("li[data-greenplum-instance-id='#{hadoop_instance_id}']").click
    end
    page.should have_content "EDC Admin added a new instance hadoop_instance"
    go_to_user_list_page
    within(".list") do
