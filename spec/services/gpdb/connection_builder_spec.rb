...
describe Gpdb::ConnectionBuilder do
  describe ".create" do
    ...
    it "requires name" do
      expect {
        Gpdb::ConnectionBuilder.create_cache!(valid_attributes.merge(:name => nil), owner)
    ...
    it "requires db connection params" do
      [:host, :port, :database].each do |attribute|
        expect {
          Gpdb::ConnectionBuilder.create_cache!(valid_attributes.merge(attribute => nil), owner)
        }.to raise_error(ActiveRecord::RecordInvalid)
      end
    end
    ...
    it "requires that a real connection to GPDB can be established" do
      ...
      begin
        Gpdb::ConnectionBuilder.create_cache!(valid_attributes, owner)
